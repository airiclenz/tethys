...
> **Versions.** Released version **2.0.0** (`code/master/globals/config.py`).
> Wire-protocol version **1** (`code/master/core/protocol.py` ↔
> `code/sensor/include/wpw_Config.h`). Sensor firmware EEPROM schema **v3**.
> Full history in [`CHANGELOG.md`](CHANGELOG.md); open work in [`TODO.md`](TODO.md).
...
```bash
cd code/sensor
pio run -t program -e TX     # via an AVR ISP programmer
```
...
with a blocking HTTP call inside the sensor's short listen window.
...
- **Wire protocol is versioned and fixed-size.** `core/protocol.py` and
  ...
  change — and re-flash everything (no backward compat).
...
`compile.sh` (`tsc`) / `compileAutomatically.sh` (watch). The build is non-strict
and currently emits with pre-existing type errors (`noEmitOnError` off — see
`TODO.md`). Use `install/deploy-static.sh` to compile + publish to the nginx
static dir.
...
- The legacy Arduino `#ifdef RX` firmware (a standalone Arduino receiver, the
  pre-Pi master) is superseded by the Raspberry Pi master and slated for removal.
