...
# Deploys the web frontend: compiles the TypeScript to JavaScript, then copies
# the static assets to the nginx-served staticcollect folder and fixes
# permissions. Use this after editing anything under web/static/ts (or the other
# static folders). It does NOT touch the systemd services - static files are
# served directly by nginx, and JS changes are picked up on the next browser
# reload. (For template changes, restart tethys-web instead: it uses Django's
