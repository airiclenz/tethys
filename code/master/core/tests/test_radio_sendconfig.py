...
# 4 - periodic (settings-only) pull: never clears the one-shot calibration flag,
#     even when delivered, so a running node's hourly pull can't consume a
#     pending calibration request (calibration stays boot-only).
def test_periodic_pull_never_clears_calibration_flag():
    obj, cleared = _make_radio(write_result=True, cached_payload=CALIB_PAYLOAD)

    obj.sendConfig(1, includeCalibration=False)

    assert cleared == []


# 5 - periodic pull strips the calibration trigger from the frame it sends, but
#     keeps the real measure-frequency / TX-power so settings still propagate.
def test_periodic_pull_strips_calibration_but_keeps_settings():
    cached = protocol.build_config_payload(120, 2, True)
    obj, cleared = _make_radio(write_result=True, cached_payload=cached)

    obj.sendConfig(1, includeCalibration=False)

    sent = protocol.parse_config_payload(obj.radio.written[-1])
    assert sent.triggerCalibration is False
    assert sent.measureFrequency == 120
    assert sent.transmissionPowerLevel == 2
    assert cleared == []
