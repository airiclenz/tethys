...
from protocol import (
    ...
    DATATYPE_CMD_GETCONFIG_PERIODIC,
    DATATYPE_CONFIG,
    PAYLOAD_SIZE,
)
...
class Radio:
    ...
    def handlePayload(self, channelNo, payload):
        ...
        try:
            ...
            # the sensor requested the configuration (boot-time: full config,
            # may carry the one-shot calibration trigger and clear it on delivery)
            ...
            # trigger -- calibration stays a boot-only operation, so a periodic
            # pull must never consume (clear) a pending calibration request.
            elif messageType == DATATYPE_CMD_GETCONFIG_PERIODIC:
                self._logger.log("Periodic config pull was requested.")
                self.sendConfig(channelNo, includeCalibration=False)
    ...
    # =============================================================================
    def sendConfig(self, channelNo, includeCalibration=True):
        ...
        if not includeCalibration:
            # Periodic, settings-only pull: deliver measure-frequency / TX-power
            # but strip the calibration trigger, and never clear a pending flag.
            # Calibration is a boot-only, user-supervised operation (see README);
            # letting an hourly pull consume it would silently break calibration.
            ...
            self._logger.log(
                "Periodic config was sent." if delivered
                else "Periodic config send was NOT acknowledged.")
            return
